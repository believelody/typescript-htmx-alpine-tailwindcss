...
router.get('/', httpMiddleware.limitQueryValidator, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (req.query.count && req.ctx?.fromHTMX) {
      const htmxRes = await productService.fetchAll(limit, count - limit) as ProductResponse;
      if (req.session) {
    ...
    const { products, total } = await productService.fetchAll(count, 0) as ProductResponse;
